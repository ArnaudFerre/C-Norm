...
def ontoToVec(onto, factor=1.0):
    ...
    for concept in onto:
        ...
        for id_ancestor, level in levelmap.items():
            vso[id_concept][d_assoDim[id_ancestor]] = factor**level #Python set 0**0 to 1
...
if __name__ == '__main__':
    ...
    serializable_vso_OneHot = dict()
    for concept in vso_OneHot.keys():
        serializable_vso_OneHot[concept] = list()
    ...
    """
    import json
    with open('VSO_OneHot_OntoBiotope_BioNLP-ST-2016.json', 'w') as outfile:
        json.dump(serializable_vso_OneHot, outfile)
    """


    """
    for i, concept in enumerate(vso_OneHot.keys()):
        print(concept)
        print(vso_OneHot[concept])
